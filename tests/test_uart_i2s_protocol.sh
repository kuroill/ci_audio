...
audio_input="$root/components/audio_in_manage/audio_in_manage_inner.c"
codec_manager="$root/components/codec_manager/codec_manager.c"
...
must '\.alpha_forget[[:space:]]*=[[:space:]]*0\.75f' "$ssp" "NN denoise smoothing is not the SDK baseline"
...
must 'cm_set_codec_alc\(HOST_MIC_RECORD_CODEC_ID,[[:space:]]*CM_CHA_LEFT,[[:space:]]*DISABLE\)' "$audio_input" "microphone hardware ALC is still enabled"
must 'cm_set_codec_adc_gain\(HOST_MIC_RECORD_CODEC_ID,[[:space:]]*CM_CHA_LEFT,[[:space:]]*20\)' "$audio_input" "microphone fixed gain is not 20 dB"
must 'sg_i2s_uplink_dropped_frames\+\+' "$audio_pre" "uplink buffer shortage is not counted"
must 'PLAY_PRE_AUDIO_CODEC_ID[[:space:]]*!=[[:space:]]*codec_index' "$codec_manager" "non-blocking uplink shortage still floods the UART log"
