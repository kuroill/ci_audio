...
- CI1306 收到 `STOP_DOWNLINK` 后只停止 PCM 转发、静音内部 DAC 并关闭 PA，不停止 IIS0 RX、IIS0 TX、麦克风采集或 AEC/SSP。
...
| `0x23` | `STOP_DOWNLINK` | ESP32 -> CI | 空 | CI 停止 PCM 转发、静音 DAC、关闭 PA，保持 IIS RX/TX 常开，回 `STATE=0x02` 并 ACK |
...
5. CI 丢弃 IIS0 RX 中尚未消费的旧帧，重新配置内部 DAC 的 PCM buffer 和 `16000 Hz / 16-bit / mono` 格式，解除静音并打开 PA；全部完成后 ACK 并发送 `STATE=0x04`。
6. ESP32 通过 I2S DOUT 写 PCM。
7. 自然播放结束时，ESP32 写完全部 PCM 后必须继续发送 40-100 ms 全零静音 PCM，用于排空 CI 接收和播放缓冲区。
8. 静音排空结束后，ESP32 必须发送 `STOP_DOWNLINK`；CI 收到后停止 RX-to-DAC 的 PCM 转发、静音 DAC 并关闭 PA，但 IIS0 RX 继续运行且由常驻任务读取并丢弃静音帧。CI 随后返回 ACK 并发送 `STATE=0x02`。`STOP_DOWNLINK` 不得停止 IIS0 RX、麦克风采集、AEC/SSP 或 I2S TX 上行。
...
AI_UART DOWNLINK stop bytes=... rx=drain pa=off
...
- [ ] 7.4：STOP_DOWNLINK 只停止 PCM 转发、静音 DAC、关闭 PA，不停止 IIS0 RX/TX
