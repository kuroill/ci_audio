...
#if USE_IR_ENABEL
static int exit_wakeup_timeout = gCiasAiotFuncParam.wake_up_continue_timeout*1000;
...
void set_state_enter_wakeup(uint32_t exit_wakup_ms)
{
    ...
    if(sys_manage_data.is_fisrt_wakeup_state)
    {
        sys_manage_data.is_fisrt_wakeup_state = false;
        gCiasAiotRunParam.is_first_wake_up = true;
    }
    else
    {
        gCiasAiotRunParam.is_first_wake_up = false;
...
void update_awake_time(void)
{
    if (sys_manage_data.wakeup_state == SYS_STATE_WAKEUP)
    {
        set_state_enter_wakeup(gCiasAiotFuncParam.wake_up_continue_timeout*1000);
    }
}
...
void set_state_exit_wakeup(void)
{
  ...
  //  ciss_set(CI_SS_CWSL_AEC_MUTE_STATE,CI_SS_CWSL_AEC_MUTE_OFF);
    ...
    sys_manage_data.is_fisrt_wakeup_state = true;
...
void exit_wakeup_deal(uint32_t asr_busy_check)
{
    ...
    if(gCiasAiotFuncParam.is_play_exit_wakeup_voice)    //对串口指令设置有效
    {
        prompt_play_by_cmd_string("<inactivate>", -1, play_exit_wakeup_done_cb,false);
    }
    else
    {
		cmd_handle_t cmd_handle = cmd_info_find_command_by_string("<inactivate>");   //直接退出唤醒不加播报
        play_exit_wakeup_done_cb(cmd_handle);
        gCiasAiotFuncParam.is_play_exit_wakeup_voice = true;
    }
...
volatile bool asr_reseult_wakup_flag = false;
...
void sys_deal_asr_msg(sys_msg_asr_data_t *asr_msg)
{
    ...
    if((MSG_ASR_STATUS_GOOD_RESULT == asr_msg->asr_status||(MSG_CWSL_STATUS_GOOD_RESULT == asr_msg->asr_status)) && (ignore_asr_msg == 0))
    {
        ...
        {
            #if AUDIO_DATA_UPLOAD_BY_UART
            mprintf("----cias_aiot_vad_end_handle---\n");
            if(cmd_handle == weakup_asr_hander)
            {	
                asr_reseult_wakup_flag = true;
            }
            else
            {
                asr_reseult_wakup_flag = false;
            ...
            uint8_t cmd_id_buf[2] = {0};
            cmd_id_buf[0]= (cmd_id & 0xff);
            cmd_id_buf[1] = ((cmd_id >> 8) & 0xff);
            cias_send_cmd_and_data(LOCAL_ASR_RESULT_NOTIFY, cmd_id_buf, 2, DEF_FILL);  // 发送本地识别通知  
            //重置录音参数
            gCiasAiotRunParam.is_vad_on_flag = false;
            gCiasAiotRunParam.local_asr_finish_flag = true;
        //    cias_aiot_vad_end_handle(1);
#endif
...
uint8_t gs_semantic_count = 0;
uint8_t gs_send_semantic_idx = 0;
int gs_play_semantic_id_list[] =  { -1,-1,-1,-1};  
...
void UserTaskManageProcess(void *p_arg)
{
    ...
    #else
    exit_wakeup_timer = xTimerCreate("exit_wakeup", pdMS_TO_TICKS(gCiasAiotFuncParam.wake_up_continue_timeout*1000),
        pdFALSE, (void *)0, exit_wakeup_timer_callback);
    #endif
    ...
    while(1)
    {
        ...
        if(pdPASS == err)
        {
            ...
            switch (rev_msg.msg_type)
            {
				...
				case SYS_MSG_TYPE_NLP:
                {
                    if(rev_msg.msg_data.asr_data.nlp_data.flag == 1)
                    {
                        asr_result_callback_nlp("。", 3221, 1, 0, 0, 0, 0);
                    }
                    else if(rev_msg.msg_data.asr_data.nlp_data.flag == 2)
                    {
                        asr_result_callback_nlp("。", 3222, 1, 0, 0, 0, 0);
                    }
                    else if(rev_msg.msg_data.asr_data.nlp_data.flag == 3)
                    {
                ...
                case SYS_MSG_TYPE_NLP_RESULT:
                {
                    uint8_t i;
                    gs_semantic_count = rev_msg.msg_data.asr_data.nlp_data.intent_count;
                    for(i=0; i<gs_semantic_count;i++)
                    {
                        cmd_handle_t cmd_handle = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent[i]);
                        sys_msg_t send_msg;
                ...
                case SYS_MSG_TYPE_AUDIO_IN_STARTED:
                {
                    ...
                    #endif
                    {
                        ...
                        if(gCiasAiotFuncParam.is_play_enter_wakeup_voice)
                        {
                            prompt_play_by_cmd_string("<welcome>", -1, NULL,true);
                        }
                        else
                        {
                            gCiasAiotFuncParam.is_play_enter_wakeup_voice = true;
                        }
                        #else
                        play_exit_wakeup_done_cb(INVALID_HANDLE);
                        #endif
                ...
                case SYS_MSG_TYPE_PLAY:
                    prompt_play_by_cmd_id(rev_msg.msg_data.play_data.play_index, 0, NULL, true);
                break;
                #if IIS_CHANNEL_ENG_CALC_EANBLE
				case SYS_MSG_TYPE_FACTORY_TEST_PLAY: 
				{
                    prompt_play_by_voice_id(FACTORY_TEST_AUDIO_ID, factory_test_play_done_callback, true);
					break;
				}
				#endif
                default:
...
void  ci_bnpu_err_print(int err_type)
{
    mprintf("bnpu err_type = %d\r\n", err_type);
}
