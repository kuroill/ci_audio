...
#endif
    //mprintf("malloc data_in_buffer = %d\r\n", sizeof(uint8_t) * READBUF_SIZE);
...
static int32_t audio_play_decode_continue(void)
{
    ...
    while (--get_frame_flag_timeout > 0)
    {
        ...
        // mprintf("===================data_in_size = %d\r\n", data_in_size);
        // for(int i = 0; i < data_in_size; i++)
        // {
        //     mprintf("%02x ", data_in_buffer[i]);
        // }
        /* 调用解码器解码 */
 ...
 * @return uint32_t 播放处理任务状态
 */
void set_audio_play_state(uint8_t play_state)
{
...
/* 注册flac解码器 */
...
#endif
#if USE_OPUS_DECODER
    registe_decoder_ops(&opus_decoder);
#endif
#endif

    /*here only one msg queue,used overwrite function*/
    audio_play_queue = audio_play_queue_create(1, sizeof(audio_play_msg_t)); // 占1K
    ...
    audio_play_task_create(task_audio_play, "task_audio_play", AUDIO_PLAYER_PROCTHREAD_STACK, NULL, 4, &audio_play_task_handle); // 基本不占内存
#if !NET_AUDIO_PLAY_BY_OPUS
                                                                                       // 占1K   
    audio_play_task_create(task_get_play_data, "task_get_play_data", AUDIO_PLAYER_DATATHREAD_STACK, NULL, 4, NULL); // 基本不占内存
#endif
    return RETURN_OK;
}
...
void task_audio_play(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        case AUDIO_PLAY_STATE_IDLE:
        {
            ...
            //  ci_logdebug(LOG_AUDIO_GET_DATA,"cmd is 0x%x\n",uxBits);
            /* 接收播放请求消息 */
            ...
            #endif
            gCiasAiotRunParam.mp3_play_finish_flag = false;
        ...
        case AUDIO_PLAY_STATE_START:
        {
            ...
            mprintf("play_msg.flash_data.arg = 0x%x\r\n", play_msg.flash_data.arg);
            uint8_t opus_head_buf[4] = {0};
            post_read_flash((char *)opus_head_buf, play_msg.flash_data.arg, 4);
            if (strncmp(opus_head_buf, "OPUS", 4) == 0)
            {
                ci_loginfo(LOG_AUDIO_PLAY, "opus file find head ...\r\n");
            }
            else
            {
                mprintf("opus file head error : %s\r\n", opus_head_buf);
                ci_logerr(LOG_AUDIO_PLAY, "opus file head error : %s\r\n", opus_head_buf);
            }
            play_msg.flash_data.arg += 4;
            uint32_t opus_play_frame_size = 0;
            post_read_flash((char *)(&opus_play_frame_size), play_msg.flash_data.arg, 4);
            mprintf("opus_play_frame_size = %d\r\n", opus_play_frame_size);
            play_msg.flash_data.arg += 4;
            play_msg.flash_data.arg += 4; // 预留四字节
            uint16_t read_opus_data_count = 0;
            uint16_t opus_frame_total_len = opus_play_frame_size / OPUS_PLAY_QUEUE_TIEM_SIZE;
            while (read_opus_data_count < opus_frame_total_len)
            {
                if (uxQueueSpacesAvailable(gCiasAiotRunParam.opus_play_queue) > 4)
                {
                    if ((xStreamBufferBytesAvailable(gCiasAiotRunParam.pcm_play_data_stream_buffer) < PLAY_BUF_GET_DATA_MIN_SIZE))
                    {
                        memset(opus_read_buf, 0, OPUS_PLAY_QUEUE_TIEM_SIZE);
                        int ret = post_read_flash((char *)(opus_read_buf), play_msg.flash_data.arg + read_opus_data_count * OPUS_PLAY_QUEUE_TIEM_SIZE, OPUS_PLAY_QUEUE_TIEM_SIZE);
                        if (xQueueSend(gCiasAiotRunParam.opus_play_queue, (uint32_t)(opus_read_buf), 0) == pdFALSE)
                        {
                            mprintf(">>>> flash read send gCiasAiotRunParam.opus_play_queue fail !<<<<\n");
                        }
                        read_opus_data_count++;
                       // mprintf("read_opus_data_count = %d\r\n", read_opus_data_count);
                    }
                }
                else
                {
                ...
                if(audio_play_state == AUDIO_PLAY_STATE_STOP)  //播放过程中停止播放处理
                {
                    break;
                }
            }
            mprintf("get flash data end..\r\n");
#else
            // cm_pcm_buffer_info_t pcm_buffer_info;
            // pcm_buffer_info.play_buffer_info.block_num = 2;
        ...
        {
#if NET_AUDIO_PLAY_BY_OPUS
            vTaskDelay(pdMS_TO_TICKS(1));   //该处必须加延时，否则本地opus播放失败
#else
        ...
        case AUDIO_PLAY_STATE_STOP:
        {
            ...
            mprintf("ciss_get(CI_SS_PLAY_STATE) = %d\r\n", ciss_get(CI_SS_PLAY_STATE));
            #if (PLAYER_CONTROL_PA)
            ...
            gCiasAiotRunParam.mp3_play_finish_flag = true;
#else
            if (decbuf != pcm_buf_no_cache)
            {
                cm_release_pcm_buffer(sg_play_device_index, CODEC_OUTPUT, decbuf);
...
/* 如果是外部请求暂停，则声卡硬件立即停止并清理缓存 */
            ...
            #endif
            ci_loginfo(LOG_AUDIO_PLAY, "play end1\n");
            mprintf("ciss_get(CI_SS_PLAY_STATE) = %d\r\n", ciss_get(CI_SS_PLAY_STATE));
            #if (PLAYER_CONTROL_PA)
                audio_play_hw_pa_da_ctl(DISABLE, true);
            #endif
