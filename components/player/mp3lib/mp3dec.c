 ...
 **************************************************************************************/
_XIF_  static int MP3FindFreeSync(unsigned char *buf, unsigned char firstFH[4], int nBytes)
{
	int offset = 0;
 ...
 * Return:      none
 **************************************************************************************/
_XIF_  static void MP3ClearBadFrame(MP3DecInfo *mp3DecInfo, short *outbuf)
{
	/*int i;
	...
	for (i = 0; i < mp3DecInfo->nGrans * mp3DecInfo->nGranSamps * mp3DecInfo->nChans; i++)
		outbuf[i] = 0;*/
	mp3DecInfo->freeBitrateSlots = 0;  //qqm,ERR_MP3_FREE_BITRATE_SYNC错误会导致下一帧解码堆栈溢出
}
...
/**************************************************************************************
 * Function:    MP3GetSaveLastInfoStructSize
 *
 * Description: 获取保存解码器现场所需的内存空间
 *
 * Inputs:      none
 *
 * Outputs:     none
...
/**************************************************************************************
 * Function:    MP3ClearLastDecodeMainData
 *
 * Description: 清除解码器内部缓存
 *
 * Inputs:      valid MP3 decoder instance pointer (HMP3Decoder)
 * 
 * Outputs:     none
...
/**************************************************************************************
 * Function:    MP3SaveLastDecodeMainData
 *
 * Description: 保存解码器现场到目标结构
 *
 * Inputs:      valid MP3 decoder instance pointer (HMP3Decoder)
 *              valid MP3SaveLastInfo pointer
 *
 * Outputs:     none
 *
 * Return:      error code, defined in mp3dec.h (0 means no error, < 0 means error)
...
{
	MP3DecInfo *mp3DecInfo = (MP3DecInfo *)hMP3Decoder;
	memcpy(saveInfo,mp3DecInfo,sizeof(MP3DecInfo));
	return ERR_MP3_NONE;
}

/**************************************************************************************
 * Function:    MP3LoadLastDecodeMainData
 *
 * Description: 恢复目标结构中信息到解码器现场
 *
 * Inputs:      valid MP3 decoder instance pointer (HMP3Decoder)
 *              valid MP3SaveLastInfo pointer
 *
...
{
	MP3DecInfo *mp3DecInfo = (MP3DecInfo *)hMP3Decoder;
	memcpy(mp3DecInfo,saveInfo,sizeof(MP3DecInfo));
	return ERR_MP3_NONE;
}
#endif