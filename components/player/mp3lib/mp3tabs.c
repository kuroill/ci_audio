...
int (* samplerateTab)[3] = NULL;
short (* bitrateTab)[3][15] = NULL;
short (* samplesPerFrameTab)[3] = NULL;
short * bitsPerSlotTab = NULL;
short (* sideBytesTab)[2] = NULL;
short (* slotTab)[3][15] = NULL;
SFBandTable (* sfBandTable)[3] = NULL;
void config_mp3tab_point(void *p)
{
	uint32_t addr = (uint32_t)p;
	samplerateTab = (int (*)[3])addr;           //3*3*4=36
	addr+=(sizeof(int)*3*3);
	bitrateTab = (short (*)[3][15])addr;           //3*3*15*2=270
	addr+=(sizeof(short)*3*3*15);
	samplesPerFrameTab = (short (*)[3])addr;    //3*3*2=18
	addr+=(sizeof(short)*3*3);
	bitsPerSlotTab = (short *)addr;         //3*2=6
	addr+=(sizeof(short)*3);
	sideBytesTab = (short (*)[2])addr;          //3*2*2=12
	addr+=(sizeof(short)*3*2);
	slotTab = (short (*)[3][15])addr;              //3*3*15*2=270
	addr+=(sizeof(short)*3*3*15);
	sfBandTable = (SFBandTable (*)[3])addr;     //(23+14)*2*3*3=666
}
#else
