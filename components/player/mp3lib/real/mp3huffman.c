 ...
 * July 2003
 *
 * mp3huffman.c - Huffman decoding of transform coefficients
 **************************************************************************************/

#include "mp3coder.h"

/* helper macros - see comments in mp3hufftabs.c about the format of the huffman tables */
 ...
 **************************************************************************************/
_XIF_  static int DecodeHuffmanPairs(int *xy, int nVals, int tabIdx, int bitsLeft, unsigned char *buf, int bitOffset)
{
	int i, x, y;
	...
	if(tabIdx < 13)
	{
		tBase = (unsigned short *)(huffTable_0 + huffTabOffset[tabIdx]);
	}
	else if(tabIdx <= 23)
	{
		tBase = (unsigned short *)(huffTable_1 + huffTabOffset[tabIdx] - 1809);
	}
	else
	{
		tBase = (unsigned short *)(huffTable_2 + huffTabOffset[tabIdx] - 3537);
	}
 ...
 **************************************************************************************/
_XIF_  static int DecodeHuffmanQuads(int *vwxy, int nVals, int tabIdx, int bitsLeft, unsigned char *buf, int bitOffset)
{
	int i, v, w, x, y;
