 ...
 * June 2003
 *
 * mp3trigtabs.c - global ROM tables for pre-calculated trig coefficients
 **************************************************************************************/

#include "mp3coder.h"
#include <stdint.h>
#ifdef USE_OUTSIDE_MP3_TABLE
int (* imdctWin)[36] = NULL;
int (* ISFMpeg1)[7] = NULL;
int (* ISFMpeg2)[2][16] = NULL;
int (* ISFIIP)[2] = NULL;
unsigned char * uniqueIDTab = NULL;
int (* csa)[2] = NULL;
int * coef32 = NULL;
int * polyCoef = NULL;
void config_mp3trigtabs_point(void *p)
{
	uint32_t addr = (uint32_t)p;
	imdctWin = (int (*)[36])addr;             //4*4*36=576
	addr += (sizeof(int)*4*36);
	ISFMpeg1 = (int (*)[7])addr;            //4*2*7=56
	addr += (sizeof(int)*2*7);
	ISFMpeg2 = (int (*)[2][16])addr;             //4*2*2*16=256
	addr += (sizeof(int)*2*2*16);
	ISFIIP = (int (*)[2])addr;               //4*2*2=16
	addr += (sizeof(int)*2*2);
	uniqueIDTab = (unsigned char *)addr; //8
	addr += (sizeof(char)*8);
	csa = (int (*)[2])addr;                  //4*8*2=64
	addr += (sizeof(int)*8*2);
	coef32 = (int *)addr;                //4*31=124
	addr += (sizeof(int)*31);
	polyCoef = (int *)addr;              //4*264=1056
}
 ...
 */
_DIF_ const int ISFMpeg2[2][2][16] = {
{
	{
