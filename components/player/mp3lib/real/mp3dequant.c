 ...
 * June 2003
 *
 * mp3dequant.c - dequantization, stereo processing (intensity, mid-side), short-block
...
_XIF_  int Dequantize(MP3DecInfo *mp3DecInfo, int gr)
{
	...
	if (fh->modeExt && (hi->gb[0] < 1 || hi->gb[1] < 1)) {
		for (i = 0; i < hi->nonZeroBound[0]; i++) {
			...
			//hi->huffDecBuf[0][i] = __SSAT(hi->huffDecBuf[0][i],30); //use cmsis core_cmInstr.h modify by QQM
		}
		for (i = 0; i < hi->nonZeroBound[1]; i++) {
			hi->huffDecBuf[1][i] = CLIPTO30(hi->huffDecBuf[1][i]);
			//hi->huffDecBuf[1][i] = __SSAT(hi->huffDecBuf[1][i],30); //use cmsis core_cmInstr.h modify by QQM
		}
	}
