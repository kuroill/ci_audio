...
typedef enum
{
    ...
    CI_SS_NNVAD_SPEECH_LEN,         //NNVAD 有效语音帧长度
    CI_SS_VOX_VAD_STATE,            //前端VOX vad状态 1-vad start 2-vad end
    CI_SS_VOX_VAD_CLEAR,            //清除vad数据状态重新计算
    CI_SS_VOX_VAD_FORCE_END,        //超时停止上传vad数据
    CI_SS_VOX_SET_END_CONFIDENCE,   //设置vox vad end 静音时间
    CI_SS_VOX_SET_THR_VAL,          //设置vox语音阈值
	CI_SS_NNVAD_IS_OPEN,            //开启nn vad 检测
    CI_SS_AEC_WORK_STATE,           //aec工作状态设置 0-非工作 1-工作状态
    CI_SS_VOX_WORK_STATE,           //vox工作状态设置
    CI_SS_NN_DENOISE_WORK_STATE,    //nn降噪工作状态
    ...
    CI_SS_AEC_IS_OPEN,
    CI_SS_AEC_DEALY_HANDLE_FRAME,   //aec延迟处理的帧数
    CI_SS_DOA_IS_OPEN,
    CI_SS_DR_IS_OPEN,
    CI_SS_BF_IS_OPEN,
...
typedef enum
{
    BNPU_AUDIO_DEAL_SLOW = 0x01,    //音频处理太慢，需要增大AUDIO_IN_BUFFER_NUM
}bnpu_err_type_t;
