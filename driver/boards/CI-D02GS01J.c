...
{
   	dpmu_set_io_reuse(PA3,FIRST_FUNCTION); /*gpio function*/
    dpmu_set_io_direction(PA3,DPMU_IO_DIRECTION_OUTPUT);
    gpio_set_output_mode(PA, pin_3);
...
{
    dpmu_set_io_reuse(PA3,FIFTH_FUNCTION);/*pwm pad enable*/
}
...
void pad_config_for_night_light(void)
{
    ...
    dpmu_set_io_reuse(PA3,FIFTH_FUNCTION);
    scu_set_device_gate(HAL_PWM1_BASE,ENABLE);
    ...
    //pwm_config.duty_max = 1000;
    pwm_init(PWM1,pwm_config);
    pwm_set_restart_md(PWM1, 0); /*重新计数生效模式：0，重新计数立即生效*/
    pwm_start(PWM1);
    night_light_w_pwm_name = PWM1;
    night_light_enable();
